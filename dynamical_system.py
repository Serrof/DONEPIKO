...
class DynamicalSystem:
    ...
    def __init__(self, mu, ecc, period, sma, Li=None):
        ...
        if mu != 0.:
            ...
            if self.Li == 1 or self.Li == 2 or self.Li == 3:
                if self.Li == 1:
                    self.x_L_normalized = numpy.array((find_L1(self.mu), 0.0, 0.0))
                elif self.Li == 2:
                    self.x_L_normalized = numpy.array((find_L2(self.mu), 0.0, 0.0))
                elif self.Li == 3:
                    self.x_L_normalized = numpy.array((find_L3(self.mu), 0.0, 0.0))
                puls = puls_oop_LP(self.x_L_normalized, self.mu)
                ...
                if Li == 4:
                    self.x_L_normalized = numpy.array((0.5 * (1.0 - 2.0 * self.mu), math.sqrt(3.) / 2., 0., 0.))
    ...
    def _Y_3bp(self, e, n, nu, half_dim):
        ...
        if half_dim == 1:
            if self.Li == 1 or self.Li == 2 or self.Li == 3:
                if e != 0.:
                    print('Y_3BP: case not coded yet')
                else:  # circular case
                    return Y_oop_LP123(nu, self.x_L_normalized, self.mu)
            else:  # Lagrange Point 4 or 5
                return Y_oop(e, nu)
        elif half_dim == 2:
            ...
            Y = numpy.zeros((6, 3))
            if self.Li == 1 or self.Li == 2 or self.Li == 3:
                Yoop = Y_oop_LP123(nu, self.x_L_normalized, self.mu)
            else:   # Lagrange Point 4 or 5
                Yoop = Y_oop(e, nu)
    ...
    def _Y_ip3bp_ds(self, nu):
        ...
        if self.ecc == 0.:
            ...
            print('_Y_ip3bp_ds: elliptical case not coded yet')
    ...
    def transition_ip3bp(self, x1_bar, nu1, nu2):
        ...
        if self.ecc == 0.:
            ...
            print('TRANSITION_IP3BP: elliptical case not coded yet')
            ...
            if half_dim == 1:
                ...
                if self.mu == 0.:
                    ...
                    if self.ecc == 0:
                        ...
                        print('PROPAGATE: 3-body elliptical in-plane case not coded yet')
                return self.transformation_inv(x2_bar, nu2)
            else:  # complete dynamics
        ...
        if BC.half_dim == 1:
            ...
            if self.mu == 0.:
                ...
                if self.ecc == 0.:
                    ...
                    print('compute_rhs: elliptical 3-body problem in-plane dynamics case not coded yet')
            ...
            z_oop = self.compute_rhs(BC_oop)
            z_ip = self.compute_rhs(BC_ip)
